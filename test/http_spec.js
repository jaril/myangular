...
describe('$http', function() {
  ...
  it('makes an XMLHTTPRequest to a given URL', function() {
    $http({
      method: 'POST',
  ...
  it('allows setting withCredentials from defaults', function() {
    ...
    $rootScope.$apply();
    var request = requests[0].withCredentials;
  ...
  it('allows transforming requests with functions', function() {
    ...
    $rootScope.$apply();
    var request = requests[0].requestBody;
  ...
  it('does not serialize blobs for requests', function() {
    ...
    var blob = new BlobBuilder(['hello'], {type: 'text/plain'});
