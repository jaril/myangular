        ...
        directives.push({
          ...
          compile: function() {
            return function link(scope, element) {
              //replace bindings with our own array to hold multiple text nodes
        ...
        if (directive.scope) {
          if (_.isObject(directive.scope)) {
            if (newIsolateScopeDirective || newScopeDirective) {
              throw 'Multiple directives asking for a new/inherited scope';
            }
...
function $HttpProvider() {
  ...
  function mergeHeaders(config) {
    ...
    return executeHeadersFns(reqHeaders, config);
  }

  function executeHeadersFns(headers, config) {
...
ASTCompiler.prototype.compile = function(text) {
  ...
  if (assignable) {
    ...
    extra = 'fn.assign = function(s,v,l) {' +
      (this.state.assign.vars.length ?
        'var ' + this.state.assign.vars.join(',') + ';' : //vars only has 1 expression anyway
        ''
      ) +
  ...
  var func = new Function(
    ...
    fnString)(
      ...
      this.$filter);
  /* jshint - W054 */
  func.literal = isLiteral(ast);
  func.constant = ast.constant;
  return func;
};
...
ASTCompiler.prototype.recurse = function(ast, context, create) {
  ...
  switch (ast.type) {
    case AST.Program:
      _.initial(ast.body).forEach(function(stmt) {
    ...
    case AST.ArrayExpression:
      /*LODASH NO LONGER SUPPORTS THIS AS THIRD ARG*/
...
},{"./filter":10,"lodash":2}],19:[function(require,module,exports){
var _  = require('lodash');
...
function $RootScopeProvider() {
  ...
  this.$get = ['$parse', function($parse) {
    ...
    // new instance of scope to be returned by the Provider's $get method
    // everything within scope.js is now private, with $rootScope the only thing exposed
